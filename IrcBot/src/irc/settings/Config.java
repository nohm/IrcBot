...
public class Config {
	...
	private static void initMainSettings() throws Exception {
		...
		sett_str.put("PORT", jo.getString("port"));
		sett_str.put("SSL", jo.getString("ssl"));
	...
	private static void initChannel(String path, String name) throws Exception {
		...
		JSONArray ign = jo.getJSONArray("ignore");
		for (int i = 0; i < ign.size(); i++) {
			JSONObject function = ign.getJSONObject(i);
			String cleaned = function.toString().replaceAll("[{}\"]", "");
			chan.putIgnore(cleaned.substring(0, cleaned.indexOf(":")), Boolean.valueOf(cleaned.substring(cleaned.indexOf(":") + 1)));
		}
	...
	public static HashMap<String, Boolean> getDefaultIgnore() throws Exception {
		JSONObject jo = (JSONObject) JSONSerializer.toJSON(TxtReader.parseTxt("config/default_config.txt"));

		HashMap<String, Boolean> defaults = new HashMap<String, Boolean>();

		JSONArray def = jo.getJSONArray("ignore");
		for (int i = 0; i < def.size(); i++) {
			JSONObject function = def.getJSONObject(i);
			String cleaned = function.toString().replaceAll("[{}\"]", "");
